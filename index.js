...
const youtube = (new YouTube("AIzaSyCc8ug8prMhsz6BBuzEhOt2b_yYQc_AaaI"))
const queue = new Map();
var servers = {};
...
var time = "<:time:569669385653059612>"
...
//NjExNjg3NTk1NDQ1MDU5NjA1.XWAMCg.cx2hidqn-yQqDRrjprw1t1Do1Bs
...
client.on("guildCreate", (guild)=> {
	...
	.setFooter("Nouveau serveur vient d'ajouter Anaïs").setTimestamp()
...
.addField("• Membres "+ `${membres}`, "``"+guild.memberCount+"`` *Membres* \n``"+ guild.members.filter(m => ! m.user.bot).size +"`` *Humains* \n``"+guild.members.filter(m => m.user.bot).size+"`` *Bot*", true)  
.addField("• Activité"+ ":beginner:", "``"+guild.members.filter(mem => mem.presence.status === "online").size+"`` *En ligne* \n``"+guild.members.filter(mem => mem.presence.status === "idle").size+"`` *Idle* \n``"+guild.members.filter(mem => mem.presence.status === "dnd").size+"`` *Ne pas déranger* \n``"+guild.members.filter(mem => mem.presence.status === "offline").size+"`` *Invisible* \n", true)      
.setFooter("Un serveur vient de retirer Anaïs").setTimestamp()
...
client.on('ready', () => {
	client.user.setActivity(`*help | ${client.guilds.size} serveurs`, { type: 'LISTENING' })
    console.log("Anaïs est prête !")
  });
...
if(message.content === prefix + "patch-notes"){
	var help_embed = new Discord.RichEmbed()
	.setColor("RANDOM")
	.setTitle(`<a:loading:606280187469889589> | Patch notes version 1.0.1 d'Anaïs`)
	.setThumbnail(message.author.avatarURL)
  ...
  if(message.content === prefix + "bot-info"){
	...
	let aicon = message.author.displayAvatarURL;
	  ...
	  .setColor("RANDOM")
	  .setThumbnail(bicon)
	  .setTitle(`${information} | Infos sur Anaïs`)
	  ...
	  .addField(`${time} <a:rainbow1:597936911239217162> Uptime`, Math.round(client.uptime / (1000 * 60 * 60)) + "Heures, " + Math.round(client.uptime / (1000 * 60)) % 60 + "Minutes, et" + Math.round(client.uptime / 1000) % 60 + "Secondes", true)
	  ...
	  .addField(`${time} <a:rainbow1:597936911239217162> Anaïs a été créé le`, client.user.createdAt)
	  .addField(`${version} <a:rainbow1:597936911239217162> Version d'Anaïs`, "1.0.1")
	  .addField(`${memoire} <a:rainbow1:597936911239217162> Utilisation de la mémoire`, `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)} MB`)
  ...
  if(message.content.startsWith(prefix + "ping")) {
	...
	var speed = speedTest();
	message.channel.send(`Je suis en train de faire votre recherche<a:loading:606280187469889589>`).then((msg) => {
	  speed.on('data',async data => {  
		let embed = new Discord.RichEmbed()
		 .setColor("RANDOM")
		 .setDescription(`${machine} | Ping d'Anaïs : ***` + ((new Date() - message.createdTimestamp)/200).toFixed(0) + ` ms***\n💓 | Ping de l\'API discord : ***` + Math.round(client.ping).toFixed(0) + `  ms***\n${serveur} | Ping du machine : ***`+data.server.ping + `  ms***\n`)
		 .setFooter(`Upload : ${data.speeds.upload} | Download : ${data.speeds.download}`)
		 //.setFooter(`Demandé par ${message.author.username}`)
		 .setTimestamp()
		 msg.edit("Voici les résultats que j'ai trouvé pour votre recherche",embed)
	   }) ;
	})
  }
  ...
  if(message.content.startsWith(prefix + "eval")) {
	...
	let code = arg.join(' ');
	if (message.author.id !== '405049567059640321') { message.channel.send(`<a:cross1:597843201012793344> Seul **Le Maître** peut utiliser cette commande`); } 
	else {
		try {
	  let ev = eval(code)
